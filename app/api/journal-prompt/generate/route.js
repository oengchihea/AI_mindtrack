...
async function generatePromptsForAI(promptInstruction) {
  ...
  try {
    ...
    console.log("generatePromptsForAI: Received raw response from Gemini API:", jsonString)
    ...
    try {
      ...
      if (parsedJson.prompts && Array.isArray(parsedJson.prompts) && parsedJson.prompts.length > 0) {
        console.log("generatePromptsForAI: Successfully parsed JSON with prompts:", JSON.stringify(parsedJson))
        return parsedJson
      }
      throw new Error("Invalid response format: 'prompts' array missing or empty")
    } catch (parseError) {
      console.error("generatePromptsForAI: Error parsing JSON from AI:", parseError)
      console.error("generatePromptsForAI: Raw JSON string was:", jsonString)
      if (jsonString.includes('"prompts":')) {
        const promptArrayMatch = jsonString.match(/"prompts":\s*(\[.*?\])/)
        if (promptArrayMatch && promptArrayMatch[1]) {
          try {
            console.log("generatePromptsForAI: Attempting fallback parsing for prompts array.")
            const prompts = JSON.parse(promptArrayMatch[1])
            return { prompts }
          } catch (e) {
      ...
      return { error: "Failed to parse JSON from AI response. Raw response: " + jsonString }
    }
  } catch (apiError) {
...
Return the prompts as a JSON object with a 'prompts' array in this exact format: {"prompts": ["prompt1", "prompt2", "prompt3"]}. DO NOT include any text before or after the JSON.`
    ...
    if (!result.prompts || !Array.isArray(result.prompts) || result.prompts.length === 0) {
      console.warn("API Route: No valid prompts generated or invalid format from AI. Result:", JSON.stringify(result))
      return NextResponse.json(
        {
          error: "No valid prompts generated or invalid format from AI.",
          details: "AI might have returned an empty, null, or malformed prompt list.",
        },
        { status: 500 },
      )
    }
    ...
    console.log("API Route: Successfully generated prompts with score. Sending response to client:", JSON.stringify(responseData))
